...
Run `tejun-desk --help` before the first desk command in a session. The tool owns the
...
An assigned desk is a private branch and worktree. Compare `tejun-desk status --assignment`
with the brief before writing. `tejun-desk sync <repo>` adopts accepted dev
changes; commit preserves a coherent checkpoint on the private desk; `tejun-desk hand-in
<repo>` admits committed work to the Team review line. None of those operations is Git
push.

If no desk was named, `tejun-desk open <repo>` either opens one or reports that the
