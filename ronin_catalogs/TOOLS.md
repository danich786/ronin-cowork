...
| `session_create` | **Create one visible Agent through the ordinary launch resolver.** Fork, fork it, launch, new Agent, and new session all mean this command, never a CLI-internal sub-agent. Supply the newborn's purpose explicitly with `--prompt`; only Campaign and Team launch context are resolved, never the caller's conversation. There is no dial option: the caller is unchanged and the newborn reads its own resolved birth packet. `session_create --help` dynamically renders provider/model choices and defaults from the canonical Campaign/provider catalog shared with UI dropdowns, so configuration changes require no maintained list. An existing name is refused rather than read or updated. |
| `session_set` | **Change named facts on one existing live session.** It updates Team membership, lead designation, or project root; a missing name is refused and the bare form points to `session_check`. |
| `team-lead` | Lead-conditional typed Team surface. `team-lead --help` lists roster read/write; project create/read/write/list/assign/return; and settled Kanban member status. Read-only Team enumeration remains `edges team`; visible delegation uses universal `session_create`. |
