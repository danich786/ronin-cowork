...
function SECTIONS() {
  return [
    { key: 'type', title: t('help.type', 'New session'), body: t('help.type_body', 'Three kinds of thing can start here. A Cowork Agent is born into Ronin with this installation, its selected features and behaviours, its reading and its Team. A bare-metal Agent is the provider\u2019s own CLI and nothing else. A terminal is a shell with no agent in it at all.') },
    ...
    { key: 'behaviours', title: t('help.behaviours', 'What a Behaviour is'), body: t('help.behaviours_body', 'A Behaviour is one short document about how the owner wants ordinary work done. An enabled behaviour is delivered at birth; an unchecked one stays readable on the shelf when the mandate points to it. Mandates starts on. The other stock choices cover buildouts, recruiting, writing things down, extra checkpoints, and reporting before fixing.') },
  ];
}
