...
function SECTIONS() {
  return [
    ...
    { key: 'behaviours', title: t('help.behaviours', 'What a Behaviour is'), body: t('help.behaviours_body', 'A Behaviour is one short document about how the owner wants ordinary work done. An enabled behaviour is delivered at birth; an unchecked one stays readable on the shelf when the mandate points to it. Every Cowork Agent receives its mandate teaching from the Ronin floor; behaviours are additional choices such as buildouts, recruiting, writing things down, extra checkpoints, and reporting before fixing.') },
  ];
}
