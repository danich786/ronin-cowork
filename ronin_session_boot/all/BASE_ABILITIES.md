...
**Fork versus spawn.** “Fork”, “fork it”, “launch”, and “new session” mean
`session_create`; give the newborn's purpose with
...
**Vocabulary:** **Fork it** / **New Agent** = `session_create`; **Tell** = `edges send`; **Wipeboard** = `edges wipeboard`;
**Show docs** = `work-record document list` then read; **Update work record** = `work-record update_record`; **New Team** = `team-lead roster write`;
**Hand in** = `worktree-desk hand-in`; **Promote** = lead review then `bin/ronin-promote <team>`; **Close session** = `session_end`.
