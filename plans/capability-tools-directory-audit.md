...
8. **Docs explain Ronin; Behaviors instruct working Agents.** `docs/` may explain the
   product, architecture, user journey and contributor contract. Operational instruction a
   working Agent must follow belongs in a Behavior, capability document, or tool help and
   must not require that Agent to discover and read `docs/` first.
...
| `ronin_catalogs/` | Cowork composition-schema owners; stock definitions and global tables | Authored stock inputs | Mixed: table entry merge for global catalogs; per-file replacement for definition directories | Keep this shelf and its defensibly owned subdirectories in place; correct ownership and assertions in situ. Risk: the root README says system scope and sometimes “shadows whole,” while `docs/shadowing.md` describes entry merge for tables and per-file replacement for definitions. The two mechanisms need distinct names. |
...
| `ronin_catalogs/behaviours/` | Behavior-definition and working-instruction owner across every application scope: floor, conditional, selectable and situational | Authored one-file definitions; body carries the Agent-facing instruction | Owner `ways/` same-name file replaces whole; new token adds | Keep as the discoverable and editable home for every Behavior, not only Campaign/Team/Agent selections. Application scope decides delivery/application; it must not make a Behavior disappear from discovery or editing. Risk: today the shelf is described as selectable composition, situational procedure is split into `ronin_sops/`, and the owner store is called `ways`; those three models obscure one Behavior system and invite scope-specific loaders. |
...
| `ronin_session_boot/` | Birth-composition owner | Authored shelf sources | Owner same-coordinate file replaces stock; new files add; levels are additive | Defends scoped, one-read teaching selected at birth. The tracked `all/README.md` is the symlinked route map described below; generated fragments are not additional authored shelf files. Physical shelf coordinates remain embedded in definitions. |
| `ronin_session_boot/all/` | Universal birth-contract owner | Authored source; glossary is rendered at birth | Same-coordinate replacement | Defends universal teaching only. `README.md` is a tracked symlink to the canonical `docs/README.md`, so the universal route map has one authored source rather than a copied boot document. Preserve and validate that source relation when packaging or resolving owner stores. |
...
| `ronin_session_boot/role/` | No current owner; retired axis | None | None | Does **not** exist in the tracked tree and has no runtime or test reference. Services’ capability contract explicitly says not to restore the retired session-role catalog. Keep it absent. |
...
10. **Retired-axis revival.** `ronin_session_boot/role/` is absent and unreferenced, but can
    be accidentally recreated from obsolete design language. Keep the retired coordinate
    named in verification or migration review without restoring a compatibility directory.
