...
2. **Definitions compose; content does not.** Installation, behaviour, and capability
   definitions name resources. They must not become alternate copies of operational or
   architectural prose.
...
6. **Provider-native skills remain provider-owned.** Ronin may point to an installed
   ...
   skill bodies into Cowork catalogs, SOPs, or boot shelves.
...
| `ronin_sops/` | House-practice owner | Authored stock guidance | Owner same-name file replaces whole; new file adds | Defends situation-selected practice and voice. Risk: discovery is informal (“found by name”), while behaviour definitions also list SOP tokens. `ronin_sops/gbrain.md` contradicts `docs/gbrain.md` on the system of record and overstates “off” as no MCP for Codex. |
| `ronin_library/` | Tool/capability reference owner | Authored stock reference | Owner same-name file replaces whole; new file adds | Defends supporting pages explicitly fetched by instructions. It is intentionally sparse. Risk: only `documents.md` exists and the boundary from SOP is conceptual, not schema-enforced; references can become unreachable or point back to an old location. |
...
| `ronin_catalogs/behaviours/gbrain.md` | selectable contribution: reading, SOP, MCP, capability-selection facts | copied MCP instructions or service health claims |
...
| `ronin_sops/gbrain.md` | Ronin’s chosen working practice when the situation arises | provisioning, upstream reference, volatile machine facts |
...
remain named `gbrain.md` under installation, behaviour, capability and practice owners,
but a reference must point directly to the right owner directory rather than pass through
a token map.
...
Keep product and repository ownership visible. Do not build one giant `resources/`
directory shared by both repositories. The conceptual target is:

```text
ronin-cowork/
  docs/                         # user + contributor explanations
  agent-resources/
    definitions/                # installation, behaviour, capability, template
    readings/                   # universal + scoped birth source
    practices/                  # SOPs
    reference/                  # tool/capability library
    presentation/               # lexicons, profiles, skins
    commands/                   # stock Agent-facing executables + metadata
  src/resource-runtime/         # resolvers, validation, projections, DTOs
  public/                       # UI consumers only

ronin-services/
  parts/<id>/                   # independently loadable runtime entries
  shared/                       # non-activating shared runtime
  installers/<id>/              # setup/doctor/uninstall/pins/snapshot adapter
  docs/                         # Services-only contracts
```

This is the target ownership layout, not an instruction to create these directories in
this audit. The important simplification is the four namespaces: definitions, delivered
knowledge, commands, and runtime parts. Presentation and docs remain consumers/supporting
context. When implementation begins, stock sources, owner stores, loaders, bundle paths,
docs, tests and build inputs move together; the old paths are removed in that change.
...
2. Move each authored stock directory and corresponding owner-store content to its target
   owner. Preserve file contents, same-name winner semantics and permissions.
...
5. Make Services `dev-sync` place only generated runtime and emit source revision/file hash
...
No alias table, compatibility reader, dual-write period, staged namespace, or lingering
deprecated directory is recommended.
