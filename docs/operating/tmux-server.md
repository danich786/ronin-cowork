...
> `docs/operating/remote-machine-health.md`, which owns the box — memory, swap, disk, the
...
`docs/architecture/session-identity.md` is the full explanation, including why one pane answers to
...
Same rule as the sibling page, and for the same reason. **Do not build a reaper, a
...
remedy in `docs/architecture/session-identity.md`, and neither is a cleanup script.
