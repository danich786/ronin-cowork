...
> its own book**, `docs/operating/tmux-server.md`, and deliberately are not here: a session
...
- **Do not kill anything on the strength of this file.** Process cleanup belongs to the
  ...
  `docs/operating/tmux-server.md`, and the tmux server is the one process on the box that
