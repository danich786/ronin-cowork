...
it. The primitive was hardened in the same pass (docs/architecture/ui.md) — this stops *causing* the
...
then **re-reads** rather than assuming the write took. See `docs/architecture/session-control-dials.md`.
...
Retained server messages remain visible in Messages. See [message delivery](../architecture/message-queue.md).
