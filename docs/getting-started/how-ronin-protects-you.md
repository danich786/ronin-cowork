...
| actual listening process | `port=$(sed -n 's/^PORT=//p' current/.env \| tail -1); port=${port:-4810}; p=$(ss -ltnp \| sed -n "s/.*:$port .*pid=\([0-9]*\),.*/\1/p" \| head -1); cat /proc/$p/cgroup` | socket holder's cgroup ends in `ronin.service`; absent or ambiguous evidence is `unknown` because `MainPID` is only the npm wrapper |
...
- **Work separation:** prompts, branches, worktrees, and coordination procedures reduce
  accidental collision; they are not security isolation ([how Worktrees decide](../architecture/worktrees.md)).
