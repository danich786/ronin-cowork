...
`docs/architecture/ui.md` is the design system. This is the compatibility layer around it.
...
4. **Where does it keep its settings, and does that file travel?** None of them are in this
   repo. A fresh install has the vendor's defaults, not yours .
...
strongest mark" holds in both shells — see the ANSI-white note in `docs/architecture/ui.md`. Change 7 or
