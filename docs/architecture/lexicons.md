...
names a skin (`docs/architecture/desk-profiles.md`).
...
(`docs/architecture/shadowing.md`): yours in the catalogs store replaces ours of the same name; a new
...
the gate — is `docs/products/kokugo.md`, the instruction every view-builder reads. Every key with
...
Anything an **agent** reads — the letter, the brief, the boot shelf, `work-record update_record` —
