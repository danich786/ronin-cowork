...
which **lexicon** (the words — `docs/architecture/lexicons.md`), which **campaign kind** the board
...
name (`docs/architecture/shadowing.md`). The directory's `README.md` carries the fields. Five ship:
...
paints in the primary campaign's. `docs/using-ronin/campaigns.md`. `GET /api/desk-profiles` serves the list (with
