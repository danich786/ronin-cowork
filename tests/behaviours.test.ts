...
test('behaviour books resolve from the stock shelf and whole-file owner shadows', async () => {
  ...
  try {
    ...
    assert.deepEqual(answer.delivered.map((row) => row.book), ['mandates', 'ways:my_way']);
    assert.equal(answer.delivered[0]?.file, path.join(process.env.RONIN_WAYS_DIR, 'mandates.md'));
    assert.equal(answer.delivered[1]?.file, path.join(process.env.RONIN_WAYS_DIR, 'my_way.md'));
