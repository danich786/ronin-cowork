...
test('behaviour books resolve from the stock shelf and whole-file owner shadows', async () => {
  ...
  await writeFile(path.join(process.env.RONIN_WAYS_DIR, 'mandates.md'), '# My mandates\n\nMine.\n');
  await writeFile(path.join(process.env.RONIN_WAYS_DIR, 'my_way.md'), '# My Way\n\nMine.\n');
  try {
    const { resolveBehaviourBooks, resolveFloorBehaviour } = await import('../src/behaviours.js');
    ...
    assert.equal(answer.delivered[0]?.file, path.join(process.env.RONIN_WAYS_DIR, 'my_way.md'));
    ...
    assert.equal((await resolveFloorBehaviour('mandates'))?.file, path.join(process.env.RONIN_WAYS_DIR, 'mandates.md'),
