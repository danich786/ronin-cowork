...
test('the catalog rows and the help of the family agree on the names', async () => {
  ...
  for (const name of ['session_end', 'session_archive', 'session_restore']) {
    assert.match(actions, new RegExp(`> \\*\\*Tool: \`${name}`), `${name} is the action's tool pointer`);
  }
  const macros = await readFile(path.join(root, 'ronin_catalogs', 'MACROS.md'), 'utf8');
  assert.match(macros, /`session_fork`/, 'session_fork is the forkit macro tool pointer');
});
