...
test('the shipped agent shelf surfaces loadouts, and no team answers', async () => {
  ...
  assert.deepEqual(assistant?.features, ['gbrain']);
...
test('a cast parses from the section format, row keys never leaking top-level', () => {
  const rows = parseTemplateAgents([
    ...
    '### two', '- **instructions:** Work.', '- **features:** ronin_worktrees, gbrain',
  ].join('\n'));
  assert.deepEqual(rows, [
    { name: 'one', instructions: 'Lead.', mandate: { reach: 'open', recruit: 'nobody', output: ['open'] }, team_lead: true, features: [] },
    { name: 'two', instructions: 'Work.', mandate: null, team_lead: false, features: ['ronin_worktrees', 'gbrain'] },
  ]);
