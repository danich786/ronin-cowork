...
test('a complete promotion tells each contributing session once and posts the moved line once', async () => {
  ...
  assert.match(told[0]![1], /certified clean: stay parked for more work, or go with session_end/);
  assert.doesNotMatch(told[0]![1], /worktree-desk close/, 'a birth desk is never closed under a live session');
});
