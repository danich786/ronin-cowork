...
// question is a stone with the right reading, behaviours are installation-filtered, the
...
const seedWith = (available) => ({ behaviours: [
  { name: 'ronin_host', label: 'Ronin Host', available: available.includes('ronin_host'), reading: '/ronin_host.md' },
  { name: 'mandates', label: 'Mandates', available: true, required: true, reading: '/mandates.md' },
  { name: 'buildout', label: 'Buildout', available: true, reading: '/buildout.md' },
], available });
...
test('every question is an ERABI stone whose reading is the saved answer', async () => {
  ...
  ], 'the stones, in the tab’s order: only available behaviours are asked, and no Control stone');
  ...
  assert.equal(readingOf(form, 'ronin_host'), 'Off');
...
test('an unavailable installation-gated behaviour is absent', async () => {
  serve(seedWith([]));
  const host = new FakeNode('div');
  renderTeamConfiguration(host, roster);
  const form = await painted(host);
  assert.equal(stone(form, 'ronin_host'), null);
  assert.ok(stone(form, 'mandates'));
});

test('Save sends one behaviour record and carries agent defaults without permissions', async () => {
  serve(seedWith(['ronin_host']));
  puts.length = 0;
  const host = new FakeNode('div');
